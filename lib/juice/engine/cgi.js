...
CGI.run = function run(application) {
  ...
  /*
  for ([k,v] in Iterator(env)) {
    system.stderr.print(k,uneval(v));
  }
  */
...
CGI.buildEnv = function buildEnv(input) {
  ...
  [env.SERVER_NAME, env.SERVER_PORT] 
    = (input.HTTP_HOST || input.SERVER_NAME)
      .split(/:/);
...
CGI._fixEnv = function _fixEnv(env, origEnv) {
  ...
  if (webserver.match(/lighttpd/)) {
    ...
    //[env.PATH_INFO, env.SCRIPT_NAME] = [env.SCRIPT_NAME, env.PATH_INFO];
