...
Application.prototype.buildParams = function buildParams(env) {
    ...
    env.juice.params = {
      get: env.QUERY_STRING ? juice.Utils.splitQueryString(env.QUERY_STRING) : {},
      post: {}
    };

    var len = env.HTTP_CONTENT_LENGTH;
    if (len && env.REQUEST_METHOD == "POST" && env.CONTENT_TYPE == 'application/x-www-form-urlencoded') {
      var body = env['jsgi.input'].read(len);
