...
var Application = function Application( module, rel_root ) {
  ...
  if (module && module.resource && module.resource.resolve) {
    this.docRoot = module.resource.resolve( rel_root || ".." );
  }
...
Application.prototype.setup = evented( "setup", function setup() {
  // compat: warn
  ...
  this.templatePath = [ this.docRoot + 'templates/' ];
...
Application.prototype.buildAction = function( url, action ) {
  ...
  if ( typeof action.action == "string" ) {
    ...
    action.action = this.controller( action.action );
  }
...
Application.prototype.loadConfig = function( ) {
  var config,
      dir = this.confDir || this.docRoot + 'conf/';

  // Load the main conf.json differently
  try {
    config = JSON.parse( filesystem.rawOpen( dir + "conf.json", "r" ).readWhole() )
  }
  catch (e) {
  ...
  filesystem.list( dir ) .forEach( function ( f ) {
    if ( f == "conf.json" || !f.match(/\.json$/) || !filesystem.isFile( dir + f ) )
      return;

    try {
      var c = JSON.parse( filesystem.rawOpen( dir + f ).readWhole() )
      f = f.replace(/\.json$/, "");
