...
Application.prototype.buildAction = function( url, action ) {
  ...
  // handle sub apps
  if ( "app" in action ) {
    return this.buildSubappAction( action.app, url );
  }
...
function buildSubappAction( app, url ) {
  // TODO take action as a param to allow overriding of interesting things
  var action = {};
  ...
  action.action = function( newurl ) {
    ...
    return require( app + "/lib/app.js" ).app.juiceApp.run( request );
  }
