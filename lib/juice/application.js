...
Application.prototype.notFound = function(request) {
  var url = request.scheme
    ...
    data = { url: url,
             ...
             css: this.errorCss },
    tt = new Template(),
    html = tt.process( this.notFoundTemplate, data );

  return {
    status: 404,
    headers: { contentType : contentType },
    body: this.format === "json" ? [ data.toSource() ] : [ html ]
  };
}
...
Application.prototype.handleError = function(request, e) {
  ...
  return {
    status: 500,
    headers: { contentType : contentType },
    body: this.format === "json" ? [ data.toSource() ] : [ tt.process( this.errorTemplate, data ) ]
  };
}
