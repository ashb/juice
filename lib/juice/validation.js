...
// This fucker is definitely going to want tests
Validation.prototype.validateField = function(spec, value) {
  ...
  for ( var k in spec.validation ) {
    ...
    if ( value === undefined ) {
      if ( spec.message ) error = spec.message;
      else if ( spec.messages && spec.messages[ test ] ) error = spec.messages[ test ];
      else if ( test in Validation.messages[this.lang]) error = Validation.messages[this.lang][ test ];
      else error = test;
